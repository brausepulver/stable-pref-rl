...
for loss_weight in 0.1 1 10; do
    for i in $(seq 1 $N_RUNS); do
        ...
        outb stage uv run train \
            ...
            "+preset.method.pref.recency_loss_coupled=true" \
            "+preset.method.pref.recency_freeze_pref_heads=false" \
            'logging.tags=[pref_ppo, experiment, pred_age, disagreement]' \
            "logging.group=pref_ppo/pred_age/relative_loss/coupled/unfrozen/loss_only/${loss_weight}"
    done
done
