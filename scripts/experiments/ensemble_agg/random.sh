...
for i in $(seq 1 8); do
    seed=$((1000 * i))
    echo "Running PrefPPO with entropy sampler, random reward aggregator and seed ${seed}"
    train \
        ${BASE_PARAMS[@]} \
        "training.seed=$seed" \
        "+preset.method.pref.ensemble_agg_fn._target_=pref_rl.config.RandomAggregator" \
        "logging.tags=[pref_ppo, experiment, ensemble_agg]" \
        "logging.group=pref_ppo/exp/agg_rnd" &
done
wait
