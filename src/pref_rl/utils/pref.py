...
class Sampler:
    ...
    def sample_segments(self, episodes: list, num_samples: int, sampler: str | dict, reward_model: callable = None, stratified: bool = False):
        method = sampler if isinstance(sampler, str) else sampler.get('type', 'uniform')
        assert method in ('uniform', 'disagreement', 'entropy', 'mixed'), f"Unknown sampling method: {self.method}"
        ...
        with torch.no_grad():
            ...
            if method == 'disagreement':
                ...
                entropy_weight = sampler.get('entropy', 0.0)
                disagreement_weight = sampler.get('disagreement', 0.0)
