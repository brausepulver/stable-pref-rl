...
class UnsupervisedCallback(RewardModifierCallback):
    ...
    def _on_step(self):
        if not (self.n_steps_start <= self.num_timesteps <= self.n_steps_start + self.n_steps_unsuper):
            return True
    ...
    def _on_rollout_end(self):
        if self.num_timesteps > self.n_steps_start + self.n_steps_unsuper:
            self._run_cleanup()
            return
